...
interface RepeatPurchaseDriversSectionProps {
  drivers: MetricItem[];
}

export default function RepeatPurchaseDriversSection({ drivers }: RepeatPurchaseDriversSectionProps) {
  ...
  // Recharts expects camelCase or specific structure
  ...
  return (
    <section id="repeat-purchase-drivers" className={styles.gridSection}>
      <div className={styles.sectionHeader}>
        <div>
          ...
          <p className={styles.sectionSubtitle}>
            Key factors driving users to make repeat purchases within the same product categories.
          </p>
        </div>
      </div>
      ...
      {hasData ? (
                ...
                >
                  ...
                  <Bar dataKey="count" fill="#7C3AED" radius={[0, 4, 4, 0]}>
                    {chartData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={index === 0 ? '#7C3AED' : '#5B21B6'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
                ...
                {item.evidence && item.evidence.length > 0 && (
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.03)', paddingTop: '0.5rem' }}>
                    ...
                    {activeEvidence === index && (
                      <p style={{ fontStyle: 'italic', fontSize: '0.75rem', color: '#D1D5DB', marginTop: '0.5rem', background: 'rgba(0,0,0,0.2)', padding: '0.5rem', borderRadius: '4px', borderLeft: '2px solid #7C3AED' }}>
                        "{item.evidence[0]}"
                      </p>
                    )}
                  </div>
        ...
        <div className={styles.emptyState}>
          ...
          <p>No repeat-purchasing drivers identified in this batch.</p>
        </div>
      )}
    </section>
  );
